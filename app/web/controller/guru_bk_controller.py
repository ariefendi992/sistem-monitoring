...
def delete_jenis_pelanggaran():
    if current_user.group == "bk":
        ...
        if not sql:
            flash("Data tidak ditemukan. Status 401", "error")
        else:
        ...
        if form.validate_on_submit():
            ...
            if sql_absen:
                ...
                data.update(
                    ...
                    tahunAjaran=[i.mengajar.tahun_ajaran.th_ajaran for i in sql_absen][
                        0
                    ],
                    ...
                    countSiswa=sql_siswa.filter_by(kelas_id=form.kelas.data.id).count(),
...
def profil_bk():
    if current_user.group == "bk":
        ...
        form.fullname.data = f"{sql_one.first_name.title()} {sql_one.last_name.title()}"
